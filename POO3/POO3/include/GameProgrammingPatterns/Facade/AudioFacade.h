...
public:
  ...
  playSfx(const std::string& sfxPath) {
    std::cout << "[AudioFacade] Solicitando reproduccion de SFX..." << std::endl;

    if (loader.loadSound(sfxPath)) {
      const int sfxChannelId = 1; // Canal reservado para SFX
      mixer.playChannel(sfxChannelId);
    }
    else {
      std::cerr << "[AudioFacade] Error al cargar el SFX: " << sfxPath << std::endl;
    }
  }
